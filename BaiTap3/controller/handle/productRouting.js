...
class ProductRouting {
    ...
    static showHome(req, res) {
        fs.readFile('./views/home.html', 'utf-8', async (err, homeHtml) => {
            if (err) {
               ...
               let a =  await ProductService.getStudents()
                homeHtml = ProductRouting.getStudent(a, homeHtml);
        ...
        if (req.method === 'GET') {
            ...
            req.on('end',async (err) => {
                if (err) {
                    ...
                    await ProductService.createStudents(data)
        ...
        if (req.method === 'GET') {
            fs.readFile('./views/student/edit.html','utf-8',async (err, editHtml) => {
                if (err) {
                    ...
                    let student = await ProductService.finById(id)
                    ...
                    editHtml = editHtml.replace('{testMarks}', student[0].idTestMarks);
                    res.writeHead(200, 'text/html');
                    res.write(editHtml);
                    res.end();
                }
            ...
            req.on('end',async (err) => {
                if (err) {
                    ...
                    await ProductService.editStudent(student, id)
                    res.writeHead(301, {'location':'/student/home'});
                    res.end();
                }
            })
    ...
    static showFormDelete(req, res, id) {
        if (req.method === 'POST') {
            let a = ProductService.deleteStudent(id);
            res.writeHead(301, {'location': '/student/home'});
            res.end();
        }
    }
